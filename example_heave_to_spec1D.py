...
import numpy as np
import xarray as xr
from Spec_functions import heave_to_spec1D,heave_to_welch_spec1D
...
ds = heave_to_spec1D(ds_raw.heave,freq_new,sample_frequency=2, detrend = True, window=True)
# Using Welch method
ds_welch = heave_to_welch_spec1D(ds_raw.heave,
...
plt.plot(ds.freq_new, ds.SPEC_new.loc[max_Hs.time],color='red',label='Resmapled-Spec., Hm0[m]:'+str(Hs_inter.loc[max_Hs.time].values.round(2)))
...
Hs_inter.plot(marker='.',label='Hm0_resample')
Hs_welch.plot(marker='.',label='Hm0_welch')
plt.xlabel('time')
plt.ylabel('[m]')
